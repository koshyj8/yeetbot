...
class InventoryMenu(menus.ListPageSource):
	...
	async def format_page(self, menu, entries):
		embed = discord.Embed(title=f"{self.user.name}'s Inventory", color = discord.Color.random())
	...
	async def shop(self, ctx):
		async with aiosqlite.connect("database\eco.sqlite") as connection:
			async with connection.cursor() as cursor:
				...
				rows = await cursor.fetchall()
		embed = discord.Embed(title="Shop",description="Use the buy command to buy an item by its id", color = discord.Color.random())
		for row in rows:
